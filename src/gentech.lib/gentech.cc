...
ostream& operator<< (ostream& OS, Liste<NukleoTyp>& List)
{
	List.Ausgabe (OS);
	return OS;
}
...
: Fitness(0) , env(env), Liste<NukleoTyp>()
...
: Fitness(0) , env(env), Liste<NukleoTyp>()
{
  ...
  if ((file=fopen (FileName, "rb")) == NULL) INT_ERR (__LINE__);
  ...
  if (NukleoLen <= sizeof (double) && NukleoLen > sizeof (NukleoTyp)) {
	cerr << "!! WARNUNG !!\n"
...
int Chromosom::Save (char *FileName) const
{
  FILE *file;
  int i;
  long len;

  if ((file=fopen(FileName,"wb")) == NULL) INT_ERR (__LINE__);
// 4-7-94, 2:51 PM
...
int Chromosom::GetNukleotidNumber (int i) const
{
  ...
  {
    // Trifft auch fuer Nukleotide gleich Null zu !!!
  ...
  Nukleotide(Nukleotide)
{
  ...
  # ifdef __BORLANDC__
    randomize();
  # endif
  if ((file=fopen(StartGenFile,"rb") ) == NULL ) 
  {
...
int Chromosomen::Save (char *FileName) const
{
  FILE *file;
  int ChromLen, ChromQuantity, i, j;
...
int Chromosomen::Evolution (double GoalFitness, char *chrptrPtkFile,
			   ...
			   )
{
  ...
  # ifndef NDEBUG
      # ifdef __MSDOS__
	  cout << "\nFreier Speicher: " << farcoreleft() << endl;
      # endif
  # endif
...
{
  if (ptrSpliceCodeInfo==NULL ||
      ptrSpliceCodeInfo->SpliceCode==NULL ||
...
int Chromosomen::Echo (void) const
{
  if (Generation == 1)
	...
	printf("\r%11d%16d%16.9lf%13.9lf",
	  GetGeneration(), TheBestEversGeneration,
	  GetAverageFitness(), GetBestFitness());
    # endif
...
void Chromosom::Ausgabe (ostream& OS)
{
...
void Chromosomen::Ausgabe (ostream& OS)
{
  ...
  for (int i=0; i<laenge(); i++) {
    ...
    OS << (((*this)[i].GetFitness()< MASCHINE::eps) ?
...
/*
ostream& operator << (ostream& OS, Liste<NukleoTyp>& Ch)
{
  Ch.Ausgabe (OS);
  return OS;
}
...
# ifdef TEST
    ...
    int main()
    {
      Chromosom test1(0, 4, 5, NULL, 50), test2(0, 4, 5, NULL, 60);
	...
	cerr << "\nopps: Operator \"==\" arbeitet nicht korekt !!";
      ...
      while (!kbhit())  {
	Chromosom test3(0, Random (0, 10), 5, NULL, Random (30, 50));
	cout << endl << test3;
	getch();
      }
      return 0;
